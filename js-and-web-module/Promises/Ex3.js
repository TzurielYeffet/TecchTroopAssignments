  ...
  return new Promise((resolved, reject) => {
    setTimeout(() => {
      if (Math.random() < 0.1) {
        ...
        resolved({ transactionId, amount, status: "success" });
      }
    }, 1500);
  });
  // TODO: Return a promise that:
...
function checkout(itemNames) {
  return checkInventory(itemNames)
    ...
    .then(totalInfo => processPayment(totalInfo.total).then(paymentInfo => ({totalInfo,paymentInfo})))
    .then(({paymentInfo}) => updateInventory(itemNames).then(()=>({paymentInfo})))
    .then(({paymentInfo}) => {
