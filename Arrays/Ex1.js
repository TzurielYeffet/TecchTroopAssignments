...
//Ex2
let filterdArr =users.filter(u =>  { return u.address.zipcode.charAt(0)==='5'}); 
console.log(filterdArr);
 
//Ex3
let filterAndMappeddArr =users.filter(u =>{ return u.address.zipcode.charAt(0)==='5'}).map(u => u.id); 
console.log(filterAndMappeddArr);

// Ex4
let temp=users.map(u =>u.name).filter(u=> u.startsWith('C'));
console.log(temp);

//Ex5
let isLive = users.every(u => u.address.city ==="South Christy");
console.log(isLive);
...
console.log(inventory.map(item => item.price * item.quantity).reduce((sum,price) => sum+price,0));

//Ex9
let studentScores = [92, 87, 76, 95, 88, 72, 91, 83, 79, 96, 85, 74, 89, 93, 81];
console.log(studentScores.reduce((counts,grade)=> {
  let letter;
  if(grade >= 90){
   letter = 'A'
}else if(grade >= 80){
...
console.log(cartItems.reduce((sum,item)=>{
        sum+= ((item.price * taxRates[item.category]+item.price)* item.quantity);  
  return sum;
},0));
